...
import { RequestConfigProps, RequestConfigState } from "../types/components.types";
import { AppContextType, ThemeContextType, QueryParam, Header, FormDataField, RequestConfigData } from "../types/app.types";
...
const RequestConfig: React.FC<RequestConfigProps> = ({ onSubmit }) => {
    ...
    const [queryParams, setQueryParams] = useState<QueryParam[]>(() => {
        if (activeSession?.requestConfig?.queryParams) return activeSession.requestConfig.queryParams;
        if (savedConfig?.queryParams) return savedConfig.queryParams;
        return [];
    });

    const [headers, setHeaders] = useState<Header[]>(() => {
        if (activeSession?.requestConfig?.headers) return activeSession.requestConfig.headers;
        if (savedConfig?.headers) return savedConfig.headers;
        return [];
    });

    const [method, setMethod] = useState<string>(() => {
        if (activeSession?.requestConfig?.method) return activeSession.requestConfig.method;
        if (savedConfig?.method) return savedConfig.method;
        return "GET";
    });

    const [bodyType, setBodyType] = useState<RequestConfigState["bodyType"]>(() => {
        if (activeSession?.requestConfig?.bodyType) return activeSession.requestConfig.bodyType;
        if (savedConfig?.bodyType) return savedConfig.bodyType;
        return "none";
    });

    const [jsonBody, setJsonBody] = useState<string>(() => {
        if (activeSession?.requestConfig?.jsonBody) return activeSession.requestConfig.jsonBody;
        if (savedConfig?.jsonBody) return savedConfig.jsonBody;
        return "{\n  \n}";
    });

    const [formData, setFormData] = useState<FormDataField[]>(() => {
        if (activeSession?.requestConfig?.formData) return activeSession.requestConfig.formData;
    ...
    }, [method, queryParams, headers, bodyType, jsonBody, formData, savedConfig, activeSession, setRequestConfig, handleSaveSession]);
    ...
    useEffect(() => {
        if (activeSession?.requestConfig) {
            ...
            if (JSON.stringify(sessionConfig) !== JSON.stringify({
                method,
                queryParams,
                headers,
                bodyType,
                ...
                setFormData(sessionConfig.formData || [{ key: "", value: "", type: "text", required: false }]);

                // Update active tab based on body type
                if (sessionConfig.bodyType === "json" || sessionConfig.bodyType === "form") {
                    setActiveTab("body");
                } else {
                    setActiveTab("params");
                }
    ...
    const updateQueryParam = (index: number, field: keyof QueryParam, value: string | boolean): void => {
        ...
        updatedParams[index] = {
            ...updatedParams[index],
            [field]: value,
            description: field === "description" ? value as string : updatedParams[index]?.description || "",
            required: field === "required" ? value as boolean : updatedParams[index]?.required || false,
        };
    ...
    const updateHeader = (index: number, field: keyof Header, value: string | boolean): void => {
        ...
        updatedHeaders[index] = {
            ...updatedHeaders[index],
            [field]: value,
            description: field === "description" ? value as string : updatedHeaders[index]?.description || "",
            required: field === "required" ? value as boolean : updatedHeaders[index]?.required || false,
        };
    ...
    const updateFormDataField = (index: number, field: keyof FormDataField, value: string | boolean): void => {
