...
const URLBuilder: React.FC<URLBuilderProps> = ({ onSubmit }) => {
    ...
    const [protocol, setProtocol] = useState<string>(() => {
        if (activeSession?.urlData?.processedURL?.startsWith("https")) return "https";
    ...
    const [segments, setSegments] = useState<Segment[]>(() => {
        const parseSegments = (segmentsStr: string) => {
            if (!segmentsStr?.trim()) return [];
            return segmentsStr
                .split("/")
                .filter(Boolean)
                .map((segment) => {
        ...
        if (activeSession?.urlData?.segments) {
            return parseSegments(activeSession.urlData.segments);
        }
        if (urlData?.segments) {
            return parseSegments(urlData.segments);
        }
        return [];
    });

    const [builtUrl, setBuiltUrl] = useState<string>(() => {
        if (activeSession?.urlData?.processedURL) return activeSession.urlData.processedURL;
        if (urlData?.processedURL) return urlData.processedURL;
        return "";
    });

    const [sessionDescription, setSessionDescription] = useState<string>(() => {
        if (activeSession?.urlData?.sessionDescription) return activeSession.urlData.sessionDescription;
    ...
    useEffect(() => {
        const initializeState = () => {
            if (activeSession?.urlData) {
                ...
                if (typeof sessionUrlData.segments === 'string' && sessionUrlData.segments.trim()) {
    ...
    const updateSegment = (index: number, field: keyof Segment | 'isDynamic', value: string | boolean) => {
        setSegments(prevSegments => {
            const updatedSegments = [...prevSegments];
            const currentSegment = { ...updatedSegments[index] };

            if (field === 'isDynamic') {
                const isDynamic = value as boolean;
                updatedSegments[index] = {
                    ...currentSegment,
                    isDynamic,
                    value: isDynamic ? "" : currentSegment.value,
                    paramName: isDynamic ? currentSegment.paramName || "" : "",
                };
            } else if (field === 'value') {
                updatedSegments[index] = {
                    ...currentSegment,
                ...
                updatedSegments[index] = {
                    ...
                    value: globalVariables[value as string] || "",
                };
