...
const AITestGenerator = ({ yamlData }) => {
  const { isDarkMode } = useTheme();
  const [requirements, setRequirements] = useState('');
  const [useOOP, setUseOOP] = useState(true);
  const [isLoading, setIsLoading] = useState(false);
  const [generatedTest, setGeneratedTest] = useState('');
  const [error, setError] = useState('');
  const editorRef = useRef(null);

  const generatePytestCode = async () => {
    setIsLoading(true);
    setError('');
...
IMPORTANT: ${useOOP ? 'Please generate the test code in object-oriented style' : 'Please generate the test code in functional style'}`;
      ...
      setError(`Failed to generate test code: ${err.message || 'Unknown error'}`);
  ...
  const generateSampleTestCode = () => {
    let endpoint = "https://api.example.com/resource";
    let method = "GET";

    try {
      const endpointMatch = yamlData.match(/url:\s*["']?(.*?)["']?$/m);
      const methodMatch = yamlData.match(/method:\s*["']?(GET|POST|PUT|DELETE|PATCH)["']?$/m);
...
def test_endpoint_status_400(role: str, param1: str, param2: str, filters: Optional[Dict], expected_status: int):
    ...
    monaco.languages.setMonarchTokensProvider('python', {
      ...
      tokenizer: {
        root: [
          ...
          // Keywords
          [/@\w+/, 'annotation'],
          [/\b(and|as|assert|break|class|continue|def|del|elif|else|except|finally|for|from|global|if|import|in|is|lambda|nonlocal|not|or|pass|raise|return|try|while|with|yield)\b/, 'keyword'],
  ...
  return (
    <div className={`p-4 ${isDarkMode ? 'bg-gray-800' : 'bg-gray-50'} rounded-lg shadow`}>
      <h2 className={`text-xl font-bold mb-4 ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>AI Test Generator</h2>
      ...
      <div className="mb-4">
        ...
        <textarea
          ...
          className={`w-full h-32 p-2 border rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500 ${isDarkMode ? 'bg-gray-700 border-gray-600 text-white' : 'bg-white border-gray-300 text-gray-900'
            }`}
        />
      </div>
      ...
      <div className="mb-4 flex items-center">
        <input
          ...
          id="use-oop"
          checked={useOOP}
          onChange={(e) => setUseOOP(e.target.checked)}
          className={`h-4 w-4 text-blue-600 focus:ring-blue-500 rounded ${isDarkMode ? 'border-gray-600' : 'border-gray-300'
            }`}
        />
        <label htmlFor="use-oop" className={`ml-2 block text-sm ${isDarkMode ? 'text-white' : 'text-gray-700'}`}>
      ...
      {error && (
        <div className={`mb-4 p-3 rounded ${isDarkMode ? 'bg-red-900 text-red-100' : 'bg-red-100 text-red-700'}`}>
          {error}
        </div>
      )}
      ...
      {generatedTest && (
        <div>
          <div className="mt-4 flex justify-between">
            ...
            <h3 className={`text-lg font-medium mb-2 ${isDarkMode ? 'text-white' : 'text-gray-700'}`}>Generated Pytest Code</h3>
            <button
          ...
          <div className={`border rounded-md overflow-hidden ${isDarkMode ? 'border-gray-600' : 'border-gray-300'}`}>
            <Editor
