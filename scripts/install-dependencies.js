...
try {
  if (platform === "win32" && arch === "x64") {
    ...
    execSync("npm install --save-dev @rollup/rollup-win32-x64-msvc@^4.21.0", {
      stdio: "inherit",
    });
  } else if (platform === "linux" && arch === "x64") {
    console.log("Installing Linux x64 Rollup dependency...");
    execSync("npm install --save-dev @rollup/rollup-linux-x64-gnu@^4.21.0", {
      stdio: "inherit",
    });
  } else {
    console.log(
      `No specific Rollup dependency for platform ${platform}-${arch}`
    );
  }
  ...
  console.log(
    "This is not critical - the build will continue with fallback options."
  );
}
